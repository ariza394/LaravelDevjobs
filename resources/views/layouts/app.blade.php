            ...
            <div class="container mx-auto md:px-0">
                <div class="flex items-center justify-around">
                    ...
                    <nav class="flex-1 text-right">
                            ...
                            @guest
                                    <a class="text-white no-underline hove:underline hover:text-gray-300 p-3" href="{{ route('login') }}">{{ __('Login') }}</a>
                                @if (Route::has('register'))
                                    <a class="text-white no-underline hove:underline hover:text-gray-300 p-3" href="{{ route('register') }}">{{ __('Register') }}</a>
                                @endif
                            @else
                                <a href="{{route('vacantes.index')}}">
                                    <span class="text-gray-300 text-sm pr-4">{{ Auth::user()->name }} Admin</span>
                                </a>
